...
Future<List<Map<String, String>>> fetchPlaceSuggestions(
  ...
  final uri = Uri.https('maps.googleapis.com', '/maps/api/place/autocomplete/json', query);
  final resp = await http.get(uri);
  if (resp.statusCode != 200) return [];
  final jsonBody = resp.body.isNotEmpty ? jsonDecode(resp.body) as Map<String, dynamic> : null;
...
Future<Map<String, dynamic>?> fetchPlaceDetails(
  ...
  final query = {'place_id': placeId, 'key': apiKey, 'fields': 'formatted_address,geometry'};
  final uri = Uri.https('maps.googleapis.com', '/maps/api/place/details/json', query);
  final resp = await http.get(uri);
...
Future<Map<String, double>?> geocodeAddress(String address, String apiKey) async {
  ...
  if (resp.statusCode != 200) return null;
  try {
    final json = resp.body.isNotEmpty ? jsonDecode(resp.body) as Map<String, dynamic> : null;
